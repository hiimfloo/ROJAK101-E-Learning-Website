...
public partial class User_Etiquette : System.Web.UI.Page
{
    protected void Page_Load(object sender, EventArgs e)
    {
        if (!IsPostBack)
        {
            LoadLessonContent(2); // Load for LessonID = 2
        }
    }
        ...
        using (SqlCommand cmd = new SqlCommand(query, conn))
        {
            ...
            while (reader.Read())
            {
                dataList.Add(new ContentItem
                {
                    ...
                    ImagePath = ResolveUrl(reader["AudioPath"].ToString()) // Use ResolveUrl for ~/ paths
                });
            }
        }
